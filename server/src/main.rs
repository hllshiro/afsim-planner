...
fn run_cli(input_json: &str) -> Result<Value, String> {
    // Binary lives at <workspace_root>/target/release/afsim-planner
    // Server's cwd is workspace_root when launched via `cargo run -p afsim-planner-server`
    let mut child = Command::new("./target/release/afsim-planner")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
